...
func _setup_shop_items() -> void:
	...
	available_items.append(
		ShopItem.new(ShopItemType.HEALTH_POTION, "Health Potion", "Restores 8 HP", 25)
	)
	available_items.append(
		ShopItem.new(ShopItemType.STRENGTH_POTION, "Strength Potion", "+1 Attack permanently", 40)
	)
	available_items.append(
	...
	if persistent_data.shop_unlocks.get("revival_amulet", false):
		available_items.append(
			ShopItem.new(
				ShopItemType.REVIVAL_AMULET,
				"Revival Amulet",
				"Revive with 1 HP if you die (one-time use)",
				100
			)
		)
	...
	if persistent_data.shop_unlocks.get("teleport_scroll", false):
		available_items.append(
			ShopItem.new(
				ShopItemType.TELEPORT_SCROLL, "Teleport Scroll", "Instantly teleport to stairs", 60
			)
		)
	...
	if persistent_data.shop_unlocks.get("blessing_scroll", false):
		available_items.append(
			ShopItem.new(
				ShopItemType.BLESSING_SCROLL, "Blessing Scroll", "Gain 50 XP instantly", 75
			)
		)
